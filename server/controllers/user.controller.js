...
export const updateUser = async (req, res, next) => {
    ...
    if (req.body.password) {
        if (req.body.password.length < 6) {
            return next(errorHandler(400, "Password length should be at least 6 characters"));
        }
    }
    ...
    if (req.body.username) {
        if (req.body.username.length < 7 || req.body.username.length > 20) {
            return next(errorHandler(400, "Username length should be at least 7 & 20 characters"));
        }
        ...
        if (!req.body.username.match(/^[a-zA-Z0-9]+$/))
            return next(errorHandler(400, "Username contain only letter and number"));
    }
    try {
